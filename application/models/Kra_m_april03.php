...
Class Kra_m extends CI_Model
{
     ...
     function getAllKra()
	 {
	  ...
	  $query = $this->db->get();
       //echo $this->db->last_query().' :: '.$current_month;
	  if($query->num_rows() > 0) {
	 ...
	 $currentYear = date('Y');
	 $q = "SELECT count(kra_id) as total FROM kra JOIN users ON users.users_id = kra.users_id AND users.status='Active' AND `kra_application_date` >= '".$currentYear."-04-01'";
