...
Class Mtr_m extends CI_Model
{
	 ...
	 /* $this->db->select('*');
	  ...
	  $query = "SELECT kra.*,kra_mtr.*,users.* FROM kra JOIN users JOIN kra_mtr ON users.users_id = kra.users_id AND users.status='Active' AND kra.kra_status='Approved' AND kra.kra_id = kra_mtr.kra_id AND kra.mtr_application_date >= '".$currentYear."-04-01' group by kra.users_id";
	 //echo $query;
